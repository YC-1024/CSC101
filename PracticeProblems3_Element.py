...
'''Wrong answer
pattern_pool_full = {128:['xxx', 'x'], 64:['xx.', '.'], 32:['x.x', '.' ], 16:['x..', '.' ], 8:['.xx', 'x' ], 4:['.x.', 'x'], 2:['..x', 'x'], 1:['...', '.']}
pattern_pool = [128, 64, 32, 16, 8, 4, 2, 1]
upperbound =  255

def cal_pattern(p_num):
    result = []
    if p_num > upperbound or p_num <= 0:
        return 'Error'
    for e in pattern_pool:
        if p_num - e >= 0:
            result.append(e)
            p_num = p_num - e
    return result
...
def cellular_automaton(string, p_num, n):
    p_list = cal_pattern(p_num)
    string_temp = string[-1] + string + string[0]
    if n == 0:
        return string
    else:
        for e in p_list:
            start = string_temp.find(pattern_pool_full[e][0])
            while start != -1:
                string_temp = string_temp[:start] + pattern_pool_full[e][1] + string_temp[start+len(pattern_pool_full[e][0])-1:]
                #string =  string[:start-1] + pattern_pool_full[e][1] + string[start-1+len(pattern_pool_full[e][0]):-1]
                start = string_temp.find(pattern_pool_full[e][0])
        print 'string_temp: ' + string_temp
        return cellular_automaton(string_temp, p_num,n-1)
        #print pattern_pool_full[e][0]
End of wrong answer'''

#Correct answer
def cellular_automaton(string, pattern_num, gen):
    patterns = {}
    ...
    for i in range(7, -1, -1):
        if pattern_num/(2**i) == 1:
            ...
            pattern_num = pattern_num - 2**i
        else:
            patterns[pattern_list[i]] = '.'
            
    #generate strings n times
